...
NEW_USER="Ubuntu" # Ubuntu User Created
MYSQL_ROOT_PASSWORD="Ubuntu2025$"
MYSQL_ALT_USER="Ubuntu.User"
MYSQL_ALT_PASSWORD="Ubuntuluser"
...
vagrant ssh $MASTER_VM -c "sudo mysql -uroot -p$MYSQL_ROOT_PASSWORD -e \"CREATE USER '$MYSQL_UBU_USER'@'localhost' IDENTIFIED BY '$MYSQL_UBU_PASSWORD';\""
...
echo "Granting superuser privileges to Ubuntu on Slave..."
...
vagrant ssh $SLAVE_VM -c "sudo mysql -uroot -p$MYSQL_ROOT_PASSWORD -e \"CREATE USER '$MYSQL_UBU_USER'@'localhost' IDENTIFIED BY '$MYSQL_UBU_PASSWORD';\""
vagrant ssh $SLAVE_VM -c "sudo mysql -uroot -p$MYSQL_ROOT_PASSWORD -e \"GRANT ALL PRIVILEGES ON *.* TO '$MYSQL_UBU_USER'@'localhost' WITH GRANT OPTION;\""
